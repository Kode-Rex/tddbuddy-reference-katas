...
8. **Single-word non-last line is right-padded to width** — `justify("longword a", 10)` returns `["longword  ", "a"]`. `"longword"` alone on a line (length 8 < width 10) is padded with two trailing spaces; `"a"` is the left-aligned last line.
